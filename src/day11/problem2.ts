    ...
    monkeys.forEach((monkey) => {
      monkey.inventory.forEach((item) => {
        ...
        const commonDenominator = monkeys.reduce(
          (total: number, monkey: Monkey): number => total * monkey.test.number,
          1,
        );
        ...
        if (newWorry % monkey.test.number === 0) {
          monkeys[monkey.test.pass].inventory.push(
            newWorry % commonDenominator,
          );
        } else {
          monkeys[monkey.test.fail].inventory.push(
            newWorry % commonDenominator,
          );
        }
      });
